...
export abstract class RecipeComponentBase implements AfterViewInit, Undoable {
   ...
   * Shift scales the live `[step]` binding up, unless the field pins its own `data-shift-step`;
   * Ctrl (Windows/Linux) or Cmd (Mac) switches to a fixed unit instead — `data-fine-step` on the
   * input if a field opts into a finer one (see stepSize.ts), else 1. Only modified presses are
   * intercepted — a plain arrow or a spinner click keeps using the browser's native stepping
   * against `[step]`. A native spinner click can't expose modifier-key state to JS, so this is
  ...
  private applyModifiedStep(e: KeyboardEvent): void {
    ...
    const shiftStep = target.dataset['shiftStep'] ? Number(target.dataset['shiftStep']) : undefined;
    const delta = stepAmountForKey(e, baseStep, fineStep, shiftStep);
